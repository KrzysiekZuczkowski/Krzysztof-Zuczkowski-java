...
public class BookDirectoryTestSuite {
    ...
    void testListBooksInHandsOf() {
        //Given
        BookLibrary bookLibrary = new BookLibrary(libraryDatabaseMock);
        LibraryUser libraryUserZeroBook = new LibraryUser("Marian",
                                                          "Brzeczyszczykiewicz",
                                                          "123446326"
        );
        LibraryUser libraryUserOneBook = new LibraryUser("Jola",
                                                          "Skolimowska",
                                                          "6234469327"
        );
        LibraryUser libraryUserFiveBooks = new LibraryUser("Karol",
                                                           "Poniewierski",
                                                           "2233466324"
        );
        ...
        when(libraryDatabaseMock.listBooksInHandsOf(libraryUserZeroBook))
                .thenReturn(resultListOf0Books);
        when(libraryDatabaseMock.listBooksInHandsOf(libraryUserOneBook))
                .thenReturn(resultListOf1Book);
        when(libraryDatabaseMock.listBooksInHandsOf(libraryUserFiveBooks))
                .thenReturn(resultListOf5Book);

        //When
        List<Book> theListOfBooks0 = bookLibrary.listBooksInHandsOf(libraryUserZeroBook);
        List<Book> theListOfBook1 = bookLibrary.listBooksInHandsOf(libraryUserOneBook);
